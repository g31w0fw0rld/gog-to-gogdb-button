...
- **[PCGamingWiki](https://www.pcgamingwiki.com/)** — compatibility, fixes, ultrawide and frame-rate notes. It searches for **the game itself**: without the edition suffix and, on a DLC, an edition or a mod, by the game it belongs to, which is where PCGamingWiki documents it.
...
- **A DLC, an edition or a mod sends PCGamingWiki to its game.** GOG says which one in three different places, and only in the v2 of its API: a mod declares it in `modifiesGames` (OpenMW points at Morrowind GOTY), a DLC in `requiresGames` — and so does a pack that bundles DLC, such as *Songs of the Past*, whose list of editions is empty — while an edition declares it in that list of editions, where the plain game is the one named *Base Edition*. The v1 does not give it away: it says a product is a `dlc` or a `pack`, never of what, and it does not mark mods at all — all 27 in the catalogue look like an ordinary `game` or `pack` there. GG.deals does sell DLC and editions separately, so it always gets the full name.
...
- **[PCGamingWiki](https://www.pcgamingwiki.com/)** —compatibilidad, arreglos, ultrapanorámico y notas de frame rate—. Busca **el juego en sí**: sin el sufijo de edición y, en un DLC, una edición o un mod, por el juego al que pertenece, que es donde PCGamingWiki lo documenta.
...
- **Un DLC, una edición o un mod mandan a PCGamingWiki a su juego.** GOG dice cuál es en tres sitios distintos, y solo en el v2 de su API: un mod lo declara en `modifiesGames` (OpenMW apunta a Morrowind GOTY), un DLC en `requiresGames` —y también un pack que agrupa DLC, como *Songs of the Past*, cuya lista de ediciones viene vacía—, mientras que una edición lo declara en esa lista de ediciones, donde el juego suelto es el que se llama *Base Edition*. El v1 no lo delata: dice que un producto es `dlc` o `pack`, nunca de qué, y de los mods ni se entera —los 27 del catálogo salen ahí como un `game` o un `pack` cualquiera—. GG.deals sí vende los DLC y las ediciones por separado, así que recibe siempre el nombre completo.
